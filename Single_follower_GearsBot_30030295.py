...
class MorseCodeDecoderRobot:
    def __init__(self):
        ...
        # Initialising sensors: One color sensor for decoding Morse code and navigation guidance,
        # and an ultrasonic sensor for detecting obstacles.
    ...
    def adjust_heading(self, color):
        # This method adjusts the robot's heading based on the color detected by its sensors.
        # It ensures the robot maintains its course along a path and reacts appropriately to color-coded instructions.

        # Set base speed for the motors. This speed can be adjusted based on the colors detected by the sensors
        # to change the robot's heading.
        base_speed = 25

        # If the detected color is green (color code 3), the robot makes a minor adjustment to the left.
        # This is done by decreasing the speed of the left motor and increasing the speed of the right motor.
        if color == 3:  # Green indicates a slight turn to the left.
            self.drive_motor_left.on(SpeedPercent(base_speed - 10), brake=False)
            self.drive_motor_right.on(SpeedPercent(base_speed + 10), brake=False)

        # If the detected color is black (color code 1), the robot makes a minor adjustment to the right.
        # This is achieved by increasing the speed of the left motor and decreasing the speed of the right motor.
        elif color == 1:  # Black indicates a slight turn to the right.
            self.drive_motor_left.on(SpeedPercent(base_speed + 10), brake=False)
            self.drive_motor_right.on(SpeedPercent(base_speed - 10), brake=False)

        # If no specific navigation color is detected, the robot continues moving forward at the base speed.
        else:
            self.drive_motor_left.on(SpeedPercent(base_speed), brake=False)
            self.drive_motor_right.on(SpeedPercent(base_speed), brake=False)
